...
def compose_group_wrapper(x_start, y_start, group_modules, module_fifo_decls, module_ext_fifos):
  """ Compose the module definition of the group wrapper module 

  Retuns a list [module_name, module_def, module_call]
  """
  module_name = 'PE_module_group_wrapper_' + str(x_start) + '_' + str(y_start)
...
def create_group_wrapper(x_start, y_start, group_modules, module_defs, top_kernel):
  ...
  [module_name, module_def, module_call] = compose_group_wrapper(x_start, y_start, 
    group_modules, module_fifo_decls, module_ext_fifos)
  ...
  for module in group_modules:
    module_offset = min(module_offset, top_kernel['module_calls'].index(module))
...
def module_grouping(output_f, prefix_content, module_defs, top_kernel, group_config):
  """
  ...
  if not group_legal:
    print('[AutoSA] Error: Unable to group modules. PE-level modules contain non-fifo' +
  ...
  for x_start in range(0, grid_x, group_config['x']):
    for y_start in range(0, grid_y, group_config['y']):
      ...
      for module_call in module_calls:
        module_name = module_call['module_name']
        if 'PE' in module_name and 'dummy' not in module_name:
          if module_call['module_ids'][0] in range(x_start, x_start + group_config['x']) \
             and module_call['module_ids'][1] in range(y_start, y_start + group_config['y']):
        ...
        if 'IO_L1' in module_name:
          ...
          if m:
            ...
            if pe_x in range(x_start, x_start + group_config['x']) \
               and pe_y in range(y_start, y_start + group_config['y']):
              group_modules.append(module_call)
...
def run(input_f, output_f, config, host='opencl'):
  ...
  module_grouping(output_f, prefix_content, module_defs, top_kernel, group_config)

if __name__ == "__main__":
  parser = argparse.ArgumentParser(description='==== AutoSA Utils: Module Grouping ====')
  parser.add_argument('-i', '--input', required=True, help='kernel file')
  parser.add_argument('-o', '--output', required=True, help='modified kernel file')
  parser.add_argument('-c', '--config', required=True, help='grouping configuration')
  parser.add_argument('--host', required=False, help='Xilinx host target: hls|opencl', default='opencl')
