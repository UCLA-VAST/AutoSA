...
//    DBGSCHDNODE(stdout, node, isl_schedule_node_get_ctx(node))
//#endif    
//    data.group = group->attached_drain_group;
...
//#endif
  ...
  if (boundary) {
    ...
    node = autosa_tree_move_down_to_io_mark(node, kernel->core, buf->level); // TODO: fix
    node = isl_schedule_node_child(node, 0);
    p = isl_printer_to_str(ctx);
  ...
  int io_level, int space_dim, int read, int boundary)
{
  ...
  node = insert_filter_trans_stmts(node, io_ids, space_dim - io_level, io_level, read,
      buf, module, kernel, gen, boundary, 0, is_buffer, fifo_suffix, group); // TODO
...
//#ifdef _DEBUG
//  printf("array_name: %s\n", group->array->name);
//  printf("read: %d level: %d\n", read, io_level);
//  //if (!strcmp(group->array->name, "C") && read && io_level == 3)
//  //  printf("here\n");
//#endif
    ...
    int boundary)
{
  ...
  DBGSCHDNODE(stdout, node, isl_schedule_node_get_ctx(node));
     ...
     */
    lower_band_num = get_band_single_schedule_val(node); // TODO
    node = isl_schedule_node_child(node, 0);
  }
#ifdef _DEBUG
  DBGSCHDNODE(stdout, node, isl_schedule_node_get_ctx(node));
...
#ifdef _DEBUG
  if (!strcmp(module->name, "L_drain_IO_L2_out"))
    printf("debug here!\n");
    ...
    __isl_keep isl_ast_build *build)
{
  ...
  stmt->u.m.boundary = extract_is_boundary(ctx, name);
    ...
    __isl_keep isl_ast_build *build)
{
  ...
  stmt->u.m.dummy = !suffixcmp(stmt->u.m.module_name, "dummy");
  stmt->u.m.pe_dummy_module = pe_dummy_module;
  stmt->u.m.lower_sched_val = extract_io_stmt_int_field(
                                ctx, name, (stmt->u.m.boundary)? 3 : 2);
    ...
    const char *name, __isl_keep isl_ast_build *build)
{
  ...
  stmt->u.f.boundary = extract_is_boundary(ctx, name);
