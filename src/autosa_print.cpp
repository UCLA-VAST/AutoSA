  ...
  int inter, int arb, int boundary)
{
  ...
  if (module->double_buffer && inter != -1) {
    ...
    if (types) {
      p = isl_printer_print_str(p, inter == 0? 
								"bool intra_trans_en" : "bool inter_trans_en");
    } else {
      p = isl_printer_print_str(p, inter == 0? 
								"intra_trans_en" : "inter_trans_en");
    }
  ...
  enum platform target)
{
  ...
  int n_lane = (group->local_array->array_type == AUTOSA_EXT_ARRAY)? group->n_lane:
        ((group->group_type == AUTOSA_DRAIN_GROUP)? group->n_lane: 
        ((group->io_type == AUTOSA_EXT_IO)? group->n_lane : 
  ...
  struct autosa_kernel_stmt *stmt, struct autosa_prog *prog, struct hls_info *hls)
{
  ...
  if (isl_vec_is_zero(group->old_dir) 
      && module->type == PE_MODULE && group->pe_io_dir == IO_INOUT) {
    pe_inout = 1;
  } else {
    pe_inout = 0;
  }
  ...
  struct autosa_kernel_stmt *stmt, struct autosa_prog *prog)
{
  ...
  if (lower) {
    ...
    if (isl_vec_is_zero(group->old_dir) 
        && lower_is_PE && group->pe_io_dir == IO_INOUT) {
      /* Add in/out suffix. */
      p = isl_printer_start_line(p);
      p = isl_printer_print_str(p, "p = isl_printer_print_str(p, \"");
